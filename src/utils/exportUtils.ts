export interface ExportOptions {export interface ExportOptions {/**
  ...
  includeCharts?: boolean;  format: 'pdf' | 'excel' | 'json'; * Enhanced Export Utilities

  includeCalculations?: boolean;

  includeRecommendations?: boolean;  includeCharts?: boolean; * Provides comprehensive export functionality for structural analysis projects

}

  includeCalculations?: boolean; * Supports PDF reports, CSV data export, and project backup

export class StructuralExporter {

  static async exportToPDF(data: any, options: ExportOptions): Promise<Blob> {  includeRecommendations?: boolean; */
  ...
  static async exportToExcel(data: any, options: ExportOptions): Promise<Blob> {import jsPDF from 'jspdf';

    return new Blob(['Excel placeholder'], { type: 'application/vnd.ms-excel' });

  }export class StructuralExporter {import 'jspdf-autotable';



  static async exportToJSON(data: any, options: ExportOptions): Promise<Blob> {  static async exportToPDF(data: any, options: ExportOptions): Promise<Blob> {import Papa from 'papaparse';

    return new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });

  }    // Placeholder implementationimport { ProjectData } from '../stores/projectStore';

}
    return new Blob(['PDF placeholder'], { type: 'application/pdf' });import { AnalysisResult } from './structuralAnalysis';

  }

// Extend jsPDF type to include autoTable

  static async exportToExcel(data: any, options: ExportOptions): Promise<Blob> {declare module 'jspdf' {

    // Placeholder implementation  interface jsPDF {

    return new Blob(['Excel placeholder'], { type: 'application/vnd.ms-excel' });    autoTable: (options: any) => jsPDF;

  }  }

}

  static async exportToJSON(data: any, options: ExportOptions): Promise<Blob> {

    return new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });export interface ExportOptions {
