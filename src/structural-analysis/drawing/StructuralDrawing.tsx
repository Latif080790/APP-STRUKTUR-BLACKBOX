import React, { useRef, useEffect } from 'react';
import { Structure3D, Element, Node } from '@/types/structural';

interface StructuralDrawingProps {
  structure: Structure3D;
  className?: string;
}

// Simple SVG-based structural drawing component
export const StructuralDrawing: React.FC<StructuralDrawingProps> = ({ 
  structure, 
  className = '' 
}) => {
  const svgRef = useRef<SVGSVGElement>(null);
  
  // Calculate viewBox based on structure bounds
  const calculateViewBox = () => {
    if (structure.nodes.length === 0) {
      return { x: 0, y: 0, width: 800, height: 600 };
    }
    
    const xCoords = structure.nodes.map(n => n.x);
    const zCoords = structure.nodes.map(n => n.z);
    
    const minX = Math.min(...xCoords);
    const maxX = Math.max(...xCoords);
    const minZ = Math.min(...zCoords);
    const maxZ = Math.max(...zCoords);
    
    const width = maxX - minX;
    const height = maxZ - minZ;
    
    // Add some padding
    const padding = Math.max(width, height) * 0.1;
    
    return {
      x: minX - padding,
      y: minZ - padding,
      width: width + padding * 2,
      height: height + padding * 2
    };
  };
  
  const viewBox = calculateViewBox();
  
  // Project 3D coordinates to 2D for SVG display (top view)
  const projectTo2D = (node: Node) => {
    return {
      x: node.x,
      y: node.z
    };
  };
  
  // Render nodes as circles
  const renderNodes = () => {
    return structure.nodes.map(node => {
      const pos = projectTo2D(node);
      return (
        <g key={`node-${node.id}`}>
          <circle
            cx={pos.x}
            cy={pos.y}
            r="0.3"
            fill="#f59e0b"
            stroke="#d97706"
            strokeWidth="0.05"
          />
          <text
            x={pos.x}
            y={pos.y - 0.5}
            textAnchor="middle"
            fontSize="0.3"
            fill="#374151"
          >
            {node.id.toString()}
          </text>
        </g>
      );
    });
  };
  
  // Render elements as lines
  const renderElements = () => {
    return structure.elements.map(element => {
      const startNode = structure.nodes.find(n => n.id === element.nodes[0]);
      const endNode = structure.nodes.find(n => n.id === element.nodes[1]);
      
      if (!startNode || !endNode) return null;
      
      const startPos = projectTo2D(startNode);
      const endPos = projectTo2D(endNode);
      
      // Determine element color based on type
      let strokeColor = '#6b7280';
      if (element.type === 'beam') {
        strokeColor = '#94a3b8';
      } else if (element.type === 'column') {
        strokeColor = '#64748b';
      } else if (element.type === 'slab') {
        strokeColor = '#9ca3af';
      }
      
      return (
        <g key={`element-${element.id}`}>
          <line
            x1={startPos.x}
            y1={startPos.y}
            x2={endPos.x}
            y2={endPos.y}
            stroke={strokeColor}
            strokeWidth="0.2"
            strokeLinecap="round"
          />
          <text
            x={(startPos.x + endPos.x) / 2}
            y={(startPos.y + endPos.y) / 2 - 0.3}
            textAnchor="middle"
            fontSize="0.25"
            fill="#374151"
          >
            {element.id.toString()}
          </text>
        </g>
      );
    });
  };
  
  // Render grid lines
  const renderGrid = () => {
    if (structure.nodes.length === 0) return null;
    
    const gridLines = [];
    const gridSize = 20;
    const gridSpacing = 1;
    
    // Get bounds
    const xCoords = structure.nodes.map(n => n.x);
    const zCoords = structure.nodes.map(n => n.z);
    const minX = Math.floor(Math.min(...xCoords));
    const maxX = Math.ceil(Math.max(...xCoords));
    const minZ = Math.floor(Math.min(...zCoords));
    const maxZ = Math.ceil(Math.max(...zCoords));
    
    // Vertical lines
    for (let i = minX - 2; i <= maxX + 2; i++) {
      gridLines.push(
        <line
          key={`v-${i}`}
          x1={i}
          y1={minZ - 2}
          x2={i}
          y2={maxZ + 2}
          stroke="#e5e7eb"
          strokeWidth="0.02"
          strokeDasharray="0.2,0.2"
        />
      );
    }
    
    // Horizontal lines
    for (let i = minZ - 2; i <= maxZ + 2; i++) {
      gridLines.push(
        <line
          key={`h-${i}`}
          x1={minX - 2}
          y1={i}
          x2={maxX + 2}
          y2={i}
          stroke="#e5e7eb"
          strokeWidth="0.02"
          strokeDasharray="0.2,0.2"
        />
      );
    }
    
    return gridLines;
  };

  return (
    <div className={`w-full h-full ${className}`}>
      <svg
        ref={svgRef}
        viewBox={`${viewBox.x} ${viewBox.y} ${viewBox.width} ${viewBox.height}`}
        className="w-full h-full bg-white"
        preserveAspectRatio="xMidYMid meet"
      >
        {/* Grid */}
        {renderGrid()}
        
        {/* Elements */}
        {renderElements()}
        
        {/* Nodes */}
        {renderNodes()}
      </svg>
      
      {structure.elements.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center p-8 bg-gray-50 rounded-lg">
            <div className="text-gray-400 mb-4">
              <svg className="w-16 h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012 2v2M7 7h10" />
              </svg>
            </div>
            <h3 className="text-lg font-medium text-gray-900 mb-2">No Structure Data</h3>
            <p className="text-gray-600">Design elements to generate structural drawing</p>
          </div>
        </div>
      )}
    </div>
  );
};